...
class OfficialRbAScorer:
    def __init__(self, device: str = "cpu"):
        # has to import after RBA_REPO is on sys.path
        ...
        args = edict({
            ...
            "eval-only": True,
            # checkpoint's config.yaml defaults to cuda, crashes on CPU torch --
            # detectron2's build_model() reads this before we get the model back,
            # so it has to be forced here, not patched after
    ...
    def _to_tensor(self, img: Image.Image) -> torch.Tensor:
        ...
        if self.input_format == "BGR":
            arr = arr[:, :, ::-1].copy()
        # detectron2 normalizes internally, so we hand over raw 0-255
...
if __name__ == "__main__":
    # sanity check: load the model, score one L&F frame, print basic stats
    import glob
